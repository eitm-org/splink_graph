...
|node_id|  <centrality_name>|cluster_id|
|-------|-------------------|----------|
...
|   e   | 0.4584903903420785|         0|
    ...
    def centrality_udf(pdf: pd.DataFrame) -> pd.DataFrame:
        ...
        ec = centrality_function(nxGraph)
        out_df = (
            pd.DataFrame.from_dict(ec, orient="index", columns=[centrality_name])
            .reset_index()
            .rename(
                columns={"index": "node_id", centrality_name: centrality_name}
            )
        )
...
def katzcentrality(sparkdf, src="src", dst="dst", cluster_id_colname="cluster_id"):
    ...
    Returns:
        node_id:
        katz_centrality: Harmonic centrality of cluster cluster_id
        cluster_id: cluster_id corresponding to the node_id
...
|node_id|katz_centrality|cluster_id|
|-------|-------------------|----------|
|   b   |                2.0|8589934592|
|   c   |                1.5|8589934592|
|   a   |                1.5|8589934592|
...
|   e   | 3.3333333333333335|         0|
    ...
    hcschema = StructType(
        [
            StructField("node_id", StringType()),
            StructField("katzcentrality", DoubleType()),
            StructField(cluster_id_colname, LongType()),
        ]
    )
    ...
    def harmc(pdf: pd.DataFrame) -> pd.DataFrame:
        nxGraph = nx.Graph()
        nxGraph = nx.from_pandas_edgelist(pdf, psrc, pdst)
        hc = katz_centrality(nxGraph)
        out_df = (
            pd.DataFrame.from_dict(hc, orient="index", columns=["katz_centrality"])
            .reset_index()
            .rename(
                columns={
                    "index": "node_id",
